...
import { CLASS_IDS, CLASSES, PERIODS, SEMESTER, SUBJECTS, SUBJECT_IDS, WEEKLY_SCHEDULE } from "../data/semester";
import {
...
export function SettingsPage() {
  ...
  const [classId, setClassId] = useState<ClassId>(CLASS_IDS[0]);
  const [subjectId, setSubjectId] = useState<SubjectId>(SUBJECT_IDS[0]);
  ...
  return (
    <div className="page-stack">
      <PageHeading
        eyebrow="05・學期設定"
        title="設定"
        description={`核對 ${CLASS_IDS.length} 班有效座號、調整需關注門檻，並保留停課、補課與國定假日的日期歷程。`}
      />

      <div className="settings-grid">
        <Panel className="settings-section settings-section--wide">
          <div className="panel-heading"><div><p className="eyebrow">{SEMESTER.label}</p><h3>班級與有效座號</h3></div><span className="count-bubble">{CLASS_IDS.reduce((sum, id) => sum + CLASSES[id].seats.length, 0)} 人</span></div>
        ...
        <Panel className="settings-section settings-section--wide">
          ...
          ) : <EmptyState icon="↔" title="尚無課表異動">固定 {WEEKLY_SCHEDULE.length} 節課表會照常顯示；有停課或補課時再新增即可。</EmptyState>}
        </Panel>
        ...
        <Panel className="settings-section settings-section--wide system-info-panel">
          ...
          <dl>
            ...
            <div><dt>固定週課表</dt><dd>{WEEKLY_SCHEDULE.length} 節／{SUBJECT_IDS.length} 科／{PERIODS.length} 個節次</dd></div>
          </dl>
