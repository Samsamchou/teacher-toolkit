...
import { CLASS_IDS, CLASSES, SEMESTER, SITE, SUBJECTS, SUBJECT_IDS, WEEKLY_SCHEDULE } from "../data/semester";
import {
...
const firstWeekEnd = weekDates(weekStartKey(SEMESTER.startDate)).at(-1) ?? SEMESTER.startDate;
...
export function RecordsPage() {
  ...
  const report = useMemo<ReportExport>(() => ({
    title: `${SITE.name}_${dateFrom}_${dateTo}`,
  ...
  const chooseThisWeek = () => {
    ...
    const end = weekDates(start).at(-1) ?? start;
