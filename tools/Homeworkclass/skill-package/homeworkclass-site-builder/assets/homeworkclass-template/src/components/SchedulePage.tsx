...
import { CLASSES, PERIODS, SEMESTER, SUBJECTS, SUBJECT_IDS } from "../data/semester";
import {
      ...
      <div className="schedule-summary-row">
        ...
        <div className="schedule-legend" aria-label="科目圖例">
          {SUBJECT_IDS.map((subjectId) => (
            <SubjectBadge key={subjectId} subjectId={subjectId} />
          ))}
        </div>
      </div>
                ...
                <tr>
                  ...
                  {dates.map((date) => (
                    <th scope="col" className={holidayByDate[date] ? "is-holiday" : ""} key={date}>
                      <span>{WEEKDAY_LABELS[getISODay(parseISO(date)) - 1]}</span>
