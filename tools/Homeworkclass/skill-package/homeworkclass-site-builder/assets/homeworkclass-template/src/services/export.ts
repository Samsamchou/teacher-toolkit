...
const spreadsheetSafeText = (value: ExportCell) => {
...
export const createXlsxReportBuffer = async (report: ReportExport) => {
  ...
  report.tables.forEach((table, index) => {
    ...
    if (table.rows.length) {
      table.rows.forEach((row) =>
        worksheet.addRow(headers.map((header) => spreadsheetSafeText(row[header]))),
      );
    } else {
...
) => JSON.stringify({ ...snapshot, exportedAt } satisfies AppSnapshot, null, 2);

export const downloadJsonBackup = (snapshot: AppSnapshot) => {
  const exportedAt = new Date().toISOString();
  const date = exportedAt.slice(0, 10);
  saveBlob(
    new Blob([createJsonBackupContent(snapshot, exportedAt)], {
      type: "application/json;charset=utf-8",
    }),
    `${SITE.name}_完整備份_${date}.json`,
  );
};
