...
export const weekDates = (weekStart: string) =>
  Array.from({ length: 5 }, (_, index) => dateKey(addDays(parseISO(weekStart), index)));
...
export const revocationForHoliday = (
  ...
  exceptions
    .filter(
      (item) =>
        item.type === "holiday-revoke" && item.targetHolidayId === holidayId,
    )
...
export const holidayConflictsForDate = (
  ...
  const addedLessons = snapshot.timetableExceptions.filter(
    (item) => item.type === "add" && item.date === date,
  ).length;
  return {
    assignments,
    classroomIncidents,
...
export const scheduleForDate = (
  ...
  const added = exceptions
    .filter((item) => item.type === "add" && item.date === date && item.replacement)
...
export const assignmentsForClassWindow = (
  ...
  const classAssignments = assignments
    .filter(
      (assignment) =>
        assignment.classId === classId && assignment.assignedDate <= selectedDate,
    )
