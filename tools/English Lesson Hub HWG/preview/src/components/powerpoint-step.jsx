  ...
  if (parsed.kind !== "valid") {
    return (
      <section className="content-card powerpoint-placeholder">
        ...
        <p>{parsed.kind === "empty" ? "尚未設定 OneDrive PowerPoint Embed。請在 Teacher Studio 貼上官方 Embed URL 或 iframe code。" : parsed.message}</p>
      </section>
    );
  }
  ...
  return (
    <section className="embed-step powerpoint-step" ref={containerRef}>
      <div className="content-card-heading">
        <div><span className="content-icon">📊</span><div><p className="eyebrow">PowerPoint for the web</p><h2>{title}</h2></div></div>
        <div className="presentation-actions powerpoint-actions">
          ...
          <a className="secondary-button" href={desktopPowerPointUrl(parsed.url)} title="需要已安裝 Microsoft PowerPoint">桌面 PowerPoint</a>
        </div>
      </div>
      <iframe
        title={title}
      ...
      />
      <p className="embed-note">先在投影片畫面內點一下，再用滑鼠左鍵播放 On Click 動畫或換頁。若顯示編輯頁或 Microsoft 拒絕內嵌，請重新取得正式 Embed code，或使用新分頁／桌面 PowerPoint。</p>
    </section>
  );
}
