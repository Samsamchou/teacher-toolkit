...
> 狀態：已確認｜首批 10 張 ImageGen 錨點生成及 QA 完成｜版本：1.9｜日期：2026-08-24
...
- **內容閘門：**question-bank/HWG7-U01-U02-18題情境任務設計表-v0.2.md 的 Q1–Q18 與 3 個圖片子題已由教師確認；加入首批產圖清冊後的文件 SHA-256 為 `8A68059D35A4D0780DC24063E40A4F3CF590CF8AB9C62096AFF4868B51B45F9F`。本次未改動已確認題目內容；後續若題目內容改動，須重新確認。
...
- **ImageGen 閘門：**教師已於 2026-08-23 回覆「確認產圖」；60 張核准清單中已完成 REF-01、8 位人物母圖與 MASCOT-01 共 10 張首批錨點，狀態為 `imagegen_anchor_batch_generated_10｜anchor_qa_passed`。其餘 50 張保持 pending。
- **目前狀態：**question-bank/HWG7-U01-U02-18題情境任務設計表-v0.2.md 的 Q1–Q18、3 個圖片子題與 60 張 ImageGen 清單均已確認；Q18 子題答對後直接完成終極任務。首批 10 張錨點已保存於 `assets/imagegen/masters/` 並通過透明／構圖 QA；Google 文件保留上一次快照，不含本次產圖清冊。
...
- **產圖閘門：**教師已回覆「確認產圖」；首批 10／60 張已生成並通過人工 QA。完整檔名、尺寸、SHA-256、共同提示詞鎖、透明背景修復與誠實邊界記錄於題目設計表第 11.10 節。
- **透明 QA：**8 位人物原始輸出實際是烙入棋盤格的 RGB，不被冒充為透明；原始與 rejected 版本均留存，canonical 人物以可重現腳本建立真 alpha，並在深色底接觸表檢查手腳、髮型、服裝與白邊。
- **誠實邊界：**REF-01 原生為 1672×941，只作內部風格基準；MASCOT-01 原生真透明並等比例降採樣到 1024×1024。其餘 50 張、網站整合、OCR／洩題掃描及 iPad 實機均未執行。
...
- 第一批 HWG7 U01+02 句型與單字、固定 8 人的 v0.3 十八題設計表、3 個圖片子題與 60 張 ImageGen 清單均已確認；首批 10 張錨點已完成，其餘 50 張是下一個美術批次，本次不再同步 Google 文件。
...
教師已回覆精確文字「確認規格」、「確認圖片清單」與「確認產圖」；本規格卡目前為 v1.9。Q1–Q18、3 個圖片子題與 60 張 ImageGen 清單已確認，且首批 10 張錨點已生成並完成 QA；這不等於其餘 50 張、網站、音訊、Firebase、部署、付費或正式通行碼已完成。後續若修改核心流程、資料、安全或內容範圍，須再更新規格並由教師確認；教師已要求本批完成後收工並關機，依專案收工流程同步既有 Git 遠端後執行。
