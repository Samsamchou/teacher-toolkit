...
> 狀態：已確認｜ImageGen 60／60 張完成｜本機遊戲化改版完成（自動測試通過）｜版本：2.2｜日期：2026-08-24
...
- 學生：國小五、六年級混合程度；弱生可用兩級提示，程度較高者由 Power／Bridge Question 與跨線索推論提高挑戰。Bonus 不屬於本版 21 個必經節點。
...
- 一個原創二水守護角色、任務地圖、關卡解鎖、提示、Power／Bridge Question、答對解析與完成畫面。
...
- 基礎路線降低閱讀負荷；Power／Bridge Question 使用多線索、圖表或推論提高挑戰。未來若另增 Bonus，必須先經題庫確認且不可阻擋本版基礎路線。
...
- **退件條件：**三題只是換名詞重複同一操作；後一關沒有增加線索數、推理步驟或遷移距離；若未來新增 Bonus，反而卡住基礎路線。
...
- **ImageGen 閘門：**教師已於 2026-08-23 回覆「確認產圖」；60 張核准清單已全部完成，狀態為 `imagegen_assets_generated_60｜step5_qa_passed`。第 52–60 項及網站實作紀錄見題目設計表第 11.12 節。
...
- **產圖閘門：**教師已回覆「確認產圖」；第 01–60 項均已生成。第 52–60 項另通過透明、安全邊界、同鏡位終局、canonical 人物本機合成、接觸表及 SHA-256 QA；完整清冊見題目設計表第 11.10–11.12 節。
- **透明 QA：**8 位人物原始輸出實際是烙入棋盤格的 RGB，不被冒充為透明；原始與 rejected 版本均留存，canonical 人物以可重現腳本建立真 alpha，並在深色底接觸表檢查手腳、髮型、服裝與白邊。
- **誠實邊界：**REF-01 原生為 1672×941，只作內部風格基準。主圖已達 60／60，50 個網站 WebP 與本機網站第一版已完成；本機沒有 OCR 引擎，僅完成檔名／alt 自動掃描與人工視覺檢查。正式 OpenAI TTS、Firebase、部署及 iPad 實機仍未執行。

### 5.10 本機網站第一版的實作決策

- **實作位置：**`site/public/`；採無外部套件的靜態 ES Modules，方便後續安全放入 Firebase Hosting 的 `public` 目錄。本輪沒有 `.openai/hosting.json`，也沒有改成其他正式託管平台。
- **作答流程：**首頁未登入可看 8 張任務卡；輸入 5–8 位測試學號可建立新 attempt。若同裝置已有未完成 attempt，首頁明確提供「繼續本次作答」與「開始新紀錄」兩條路，避免重新整理／旋轉／短暫離線造成資料遺失。
- **Energy 決策：**整次 21 節點共用單一 Energy，起始 50，不在 Mission 間重設，也不封頂；18 主題題採 +10／+5／+2／−3，3 圖片子題不影響 Energy。
- **進度決策（v2.2 取代舊版）：**學生端改以六座守護站與六格物品欄呈現敘事進度，不再顯示 `主題題 x/18`、`必要節點 y/21`、Mission 1–6 或內部題號；內部資料與教師報表仍保留 18／21 計數及節點 ID。圖片子題不計入 18，但必須通過才能取得故事元件。
- **Mission 6 決策：**Q18 主題題後進入 by boat 圖片子題；通過後先取得 Water Route Key，再把六個元件組成 Final Welcome Pass，最後開門。六區表中的「Final Welcome Pass」不再被誤解成第六題組直接掉落的單一元件。
- **教師後台邊界：**開發版只提供明確標示的 localStorage 測試報表與 CSV。六碼輸入不接受任何值，避免把前端假驗證冒充正式安全後台；待 Firebase 後端、Secret、失敗限制與限時 session 確認後才啟用。
- **語音邊界：**開發版用裝置 `speechSynthesis`、rate 0.8 試聽並顯示揭露；正式版仍必須使用已核准的 OpenAI 靜態音檔與受控 SSML 編譯，不把裝置語音冒充正式成果。
- **安全基線：**`site/firestore.rules` 預設 deny-all；`firebase-config.example.js` 為空值且不含秘密。未綁定 Firebase 專案、未登入、未發布 Rules 或 Hosting。
- **測試證據（v2.2 更新）：**6 座守護站、18 主題題、21 節點、A／B／C 各 6 題、21 組中英任務引導、計分公式、Energy、續答、學生畫面與 UI 契約測試全部通過；本機首頁、程式、樣式、2 個 Comic Relief WOFF2 與 67 個 WebP 共 73 個路徑皆 HTTP 200。
## 6. 資料、安全與權限
...
- 完成正解、錯解、重試、兩級提示、解鎖、返回、重新整理及同步失敗恢復測試；本版不含未定義的 Bonus 節點。
...
- 全站採「二水水路守護站 HUD＋日式動漫角色對話」；首頁、六站地圖、任務說明、題目、提示、回饋、獎勵與結尾都使用一致遊戲介面。
- 頂端只呈現 Energy 能量條、六座站名與鎖定／目前／完成圖示、六格任務物品欄；學生端不顯示 `x/18`、`y/21`、Mission 1–6、題目 ID、Q 編號或節點數，A／B／C 選項字母保留。
- Energy 精確數值及既有計分公式不變；能量條以 0–100 填充，90 以上滿格發光，超過 100 仍顯示真實數值。
- 所有英文使用隨站提供的 Comic Relief WOFF2，中文維持易讀中文字型；系統 emoji 改為一致圖示。
- 21 個必經節點都提供一至兩行永遠可見、只說明情境與目標且不洩漏答案的中文引導；資料欄位統一為 `contextEn`、`contextZh`、`missionGoalZh`。
- 證據使用線索掃描器、選項使用遊戲答案卡；動畫以 180–450ms 的能量變化、站點解鎖、物品入槽與角色回饋為主，並支援減少動態效果。
- 1024×768 橫向採緊湊 HUD，圖片辨識子題採滿寬三圖版型；觸控目標至少 48×48px，支援 Safari 安全區與 Retina 圖。
- 教師後台沿用相同色盤但以報表可讀性為先；本次不新增 ImageGen 圖片，也不改動題目正解、節點順序、提示、計分或 attempt 規則。
...
- HWG7 U01+02 句型與單字、固定 8 人的 v0.5 十八題設計表、3 個圖片子題與 60 張 ImageGen 清單均已確認並完成；目前下一個內容閘門是正式 TTS 試聽、Firebase 名冊／保留期限／六碼後端與 iPad 實機驗收，本次不更新 Google 文件。
...
### 9.2 本機實作與自動驗證（2026-08-24）

- `npm test`：content-contract、game-rules、storage-resume、app-rendering-contract、ui-contract 全部 PASS。
- `npm run check`：`app.js`、`data.js`、`storage.js`、`game-rules.js` 全部通過語法檢查。
- 本機 HTTP 讀回：首頁、3 個前端檔案、2 個 Comic Relief WOFF2 與 67 個 WebP，共 73 個路徑，失敗 0。
- 本輪沒有新增 ImageGen 主圖、沒有改動題目正解／順序／提示／計分公式，也沒有 Firebase 綁定、正式 TTS、部署或 GitHub 推送。
- 1024×768、安全區、48px 觸控目標、滿寬三圖與 reduced-motion 已通過靜態 UI 契約；實際學校 iPad Safari 的視覺與觸控驗收仍待教師實機測試。

## 確認方式

教師已回覆精確文字「確認規格」、「確認圖片清單」與「確認產圖」；本規格卡目前為 v2.2。Q1–Q18、3 個圖片子題、60 張 ImageGen 主圖與遊戲化改版規格均已確認，`site/` 本機遊戲化版已完成並通過自動化與 HTTP 讀回驗證。題目設計表 v0.5 SHA-256 為 `044331625C2E963EA136304920133ABAA7BEF1DF0C96565369A1F6F7BBA7B299`。這不等於正式 OpenAI TTS、全批 OCR、Firebase、部署、付費、正式六碼或 iPad 實機已完成；後續若修改核心流程、資料、安全或內容範圍，仍須更新規格並由教師確認。
...
- v2.1：完成步驟 5，主圖達 60／60；建立 50 個 WebP 與本機網站第一版，固定單一 attempt Energy、18／21 雙進度、首頁新／續紀錄分流、Mission 6 元件順序及正式 Firebase／TTS 安全邊界。
- v2.2：教師確認並完成學生端全面遊戲化改版；固定二水水路守護站 HUD、Comic Relief、隱藏題號與技術節點、21 節點中英情境引導、六站／六物件敘事進度、緊湊 iPad HUD 與滿寬圖片子題；內容、計分、續答、畫面契約、UI 契約及語法檢查通過。
