...
class GameSound {
  ...
  collect(points){if(!this.context||this.context.state!=='running'||this.settings.muted)return;const c=this.context,t=c.currentTime,osc=c.createOscillator(),gain=c.createGain();osc.type=points<0?'sine':'triangle';osc.frequency.setValueAtTime(points<0?240:660,t);osc.frequency.exponentialRampToValueAtTime(points<0?120:1000,t+.16);gain.gain.setValueAtTime(this.settings.scratchVolume*.22,t);gain.gain.exponentialRampToValueAtTime(.0001,t+.2);osc.connect(gain).connect(c.destination);osc.start(t);osc.stop(t+.22);const voice={osc,gain};this.voices.push(voice);osc.onended=()=>{osc.disconnect();gain.disconnect();this.voices=this.voices.filter(v=>v!==voice);};}
