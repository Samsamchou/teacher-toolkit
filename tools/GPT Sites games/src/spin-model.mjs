...
export function createGame(count){if(!Number.isInteger(count)||count<2||count>6)throw new Error('Choose 2–6 teams.');return {phase:'ready',count,team:null,round:1,history:[],bag:[],sector:null,question:null,task:null,angle:0,die:null,early:false};}
...
export function completedThisRound(s,team){return s.history.some(h=>h.round===s.round&&h.team===team);}
export function advance(s,a){
 if(a.type==='SELECT_TEAM'&&s.phase==='ready'&&Number.isInteger(a.team)&&a.team>=0&&a.team<s.count&&!completedThisRound(s,a.team))return {...s,team:a.team};
 if(a.type==='END'&&s.phase!=='finished')return {...s,phase:'finished',early:true};
 if(a.type==='SPIN'&&s.phase==='ready'&&s.team!==null&&!completedThisRound(s,s.team)&&Number.isInteger(a.sector)&&a.sector>=0&&a.sector<8&&Number.isInteger(a.question)&&a.question>=0&&a.question<7){const target=a.sector*45+22.5;return {...s,phase:'spinning',sector:a.sector,question:a.question,angle:s.angle+2160+((target-s.angle%360+360)%360)};}
 ...
 if(a.type==='NEXT'&&s.phase==='awarded'){
  const complete=Array.from({length:s.count},(_,t)=>completedThisRound(s,t)).every(Boolean);
  const finished=complete&&s.round===6;
  return {...s,phase:finished?'finished':'ready',team:null,round:complete&&!finished?s.round+1:s.round,die:null};
 }
 return s;
}
