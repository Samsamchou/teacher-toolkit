...
export function validateManifest(m) {
  ...
  for (const game of m.games) if (typeof game.name !== 'string' || !game.name.trim() || (!['scratch','beach','spin','basketball','unscramble','plinkoh'].includes(game.id) && !safeUrl(game.url))) throw new Error('Invalid game in backup.');
  return m;
}
