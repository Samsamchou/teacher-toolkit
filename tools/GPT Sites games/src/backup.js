...
export async function restoreBackup(backup,progress){
  ...
  try{
    ...
    for(const g of m.games.filter(g=>!['scratch','beach','spin','basketball'].includes(g.id))){const id=crypto.randomUUID();await data.put('games',{id,name:g.name,description:String(g.description||''),url:safeUrl(g.url),order:existing.length+created.length,hidden:!!g.hidden,builtin:false});created.push(['games',id]);}
