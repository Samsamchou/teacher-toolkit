...
function usePoll(action,payload,student=false){
 const [value,setValue]=useState(null),[error,setError]=useState(''),[online,setOnline]=useState(false);const ref=useRef(payload);ref.current=payload;
...
function GroupRecords({room,index}){
 ...
 return <><div className="ul-groups">{Array.from({length:room.maxGroups},(_,i)=>{const g=Object.values(room.groups).find(g=>g.number===i+1);const attempts=g?.attempts.filter(a=>a.questionIndex===index)||[],last=attempts.at(-1);return <article key={i} className={`ul-group ${last?.correct?'correct':attempts.length>=5?'exhausted':last?'wrong':''}`}><header><strong>GROUP {i+1}</strong><span>{last?.correct?'✓ Correct':attempts.length>=5?'✕ 5 tries':last?'↻ Try again':g?(room.phase==='ended'?'No answer':'Waiting'):'Not joined'}</span></header><small>{g?.members.join(' · ')||'等待加入'}</small><div className="ul-answer">{room.showAnswers?(last?last.lines.map((line,j)=><div key={j}>{line}</div>):'—'):'••• 答案已遮蔽'}</div><footer><span>{attempts.length} / 5</span>{attempts.length>0&&<button aria-label={`查看第${i+1}組歷次紀錄`} onClick={()=>setHistory(g.id)}>歷次紀錄 ↗</button>}</footer></article>;})}</div>{history&&<div className="ul-modal" role="dialog" aria-modal="true" aria-label="歷次作答紀錄"><div className="ul-history-panel"><h2>GROUP {room.groups[history]?.number} · 第 {index+1} 題</h2><p>{room.groups[history]?.members.join(' · ')}</p>{room.groups[history]?.attempts.filter(a=>a.questionIndex===index).map(a=><p key={a.id}>#{a.number} {a.correct?'✓ Correct':'↻ Try again'}<br/>{room.showAnswers?a.lines.join(' '):'••• 答案已遮蔽'}<small>{new Date(a.at).toLocaleTimeString('zh-TW',{hour12:false})}</small></p>)}<button autoFocus onClick={()=>setHistory(null)}>關閉紀錄</button></div></div>}</>;
}

function TeacherRoom({roomId,onBack}){
 const {value:room,setValue,error,online}=usePoll('teacherState',{roomId}),[busy,setBusy]=useState(false),[actionError,setActionError]=useState(''),[qr,setQr]=useState(true),[recordIndex,setRecordIndex]=useState(null);
...
function StudentPlay({credentials,onLeave}){
 const {value:room,setValue,error,online}=usePoll('studentState',credentials,true),[draft,setDraft]=useState([[],[]]),[selectedRow,setSelectedRow]=useState(0),[pending,setPending]=useState(()=>readSaved(`ul-pending:${credentials.roomId}`)),[busy,setBusy]=useState(false),[message,setMessage]=useState(''),[feedback,setFeedback]=useState(null);
 const sound=useClassSound('student',room?.phase==='open');
 const currentQuestion=useRef(null),submitLock=useRef(false),feedbackTimer=useRef(null);
 ...
 function celebrate(correct){setFeedback(correct?'correct':'wrong');clearTimeout(feedbackTimer.current);feedbackTimer.current=setTimeout(()=>setFeedback(null),correct?2600:1600);sound.cue(correct);}
 useEffect(()=>{if(pending&&room){const accepted=room.group.attempts.find(a=>a.id===pending.attemptId);if(accepted){setPending(null);saveLocal(`ul-pending:${credentials.roomId}`,null);setMessage('Saved ✓');celebrate(accepted.correct);}else if(room.questionIndex!==pending.questionIndex||room.phase!=='open'){setPending(null);saveLocal(`ul-pending:${credentials.roomId}`,null);setMessage('This question is locked. An unsent answer was not counted.');}}},[room]);
 ...
 async function submit(retry=false){
  ...
  try{const data=await activity('submit',body,true);setPending(null);saveLocal(`ul-pending:${credentials.roomId}`,null);setValue(data.room);setMessage('Saved ✓');celebrate(data.attempt.correct);}
  catch(e){setMessage(e.message);if(e.status&&e.status<500){setPending(null);saveLocal(`ul-pending:${credentials.roomId}`,null);}}
