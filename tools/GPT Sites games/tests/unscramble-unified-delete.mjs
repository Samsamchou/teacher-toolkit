...
const base='http://127.0.0.1:5192',results=[];
...
await row(keep.className).click({button:'right'});await p.screenshot({path:'qa/unified-unscramble-fastload-20260915/record-context-menu.png',fullPage:true});await browser.close();
await api('end',{roomId:keep.id,revision:keep.revision});await api('deleteRoom',{roomId:keep.id});
await fs.writeFile('qa/unified-unscramble-fastload-20260915/record-delete-results.json',JSON.stringify({passed:true,environment:'local emulators only',results},null,2));console.log(results);
