...
const base='http://127.0.0.1:5184',out=new URL('../qa/unscramble-20260915/',import.meta.url);await mkdir(out,{recursive:true});
...
try{
 ...
 await check('Teacher creates a new eight-group class and QR join link',async()=>{
  await teacher.locator('.ul-deck').filter({hasText:'家人句型｜七題示範'}).first().getByRole('button',{name:'建立新場次 →',exact:true}).click();await teacher.getByLabel('班級名稱',{exact:true}).fill('QA 8組即時驗證');await teacher.getByRole('button',{name:'建立場次',exact:true}).click();await expect(teacher.locator('.ul-qr img')).toBeVisible();const link=await teacher.getByRole('link',{name:'學生加入連結 ↗'}).getAttribute('href');roomId=new URL(link).searchParams.get('room');assert.ok(roomId);await teacher.screenshot({path:new URL('teacher-lobby.png',out).pathname.replace(/^\/(\w:)/,'$1'),fullPage:true});
 });
 await check('Eight independent student contexts join with two or three identifiers; preview remains locked',async()=>{
  for(let i=0;i<8;i++){const c=await browser.newContext({viewport:{width:1024,height:768},hasTouch:true});contexts.push(c);const p=await c.newPage();students.push(p);watch(p);}
  ...
  credentials=await Promise.all(students.map(getCred));const r=(await api('teacherState',{roomId})).room;assert.equal(Object.keys(r.groups).length,8);assert.ok(Object.values(r.groups).some(g=>g.members.length===3));await expect(teacher.locator('.ul-header')).toContainText('8/8',{timeout:15000});
 });
 await check('Duplicate members, ninth group, cross-group access and teacher mutations are rejected',async()=>{
 ...
 await check('Teacher Next unlocks all eight students and pictures are visible',async()=>{
 ...
 await check('Wrong animation, correction, five-attempt cap and seven simultaneous correct records',async()=>{
  ...
  await Promise.all(students.slice(2).map(p=>answer(p,correct(0))));await expect(teacher.locator('.ul-group.correct')).toHaveCount(7,{timeout:15000});await expect(teacher.locator('.ul-group.exhausted')).toHaveCount(1);await teacher.screenshot({path:new URL('teacher-live-results.png',out).pathname.replace(/^\/(\w:)/,'$1'),fullPage:true});
 });
