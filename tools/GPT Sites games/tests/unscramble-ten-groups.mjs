...
const base='http://127.0.0.1:5184',out=new URL('../qa/unscramble-ten-groups-20260915/',import.meta.url);await mkdir(out,{recursive:true});
...
try{
 ...
 await check('Duplicate members, sixteenth group, cross-group access and teacher mutations are rejected',async()=>{
  await assert.rejects(()=>api('createRoom',{deckId:deck.id,className:'QA invalid cap',maxGroups:16}),e=>e.status===400);
 ...
 await check('Wrong animation, correction, five-attempt cap and nine simultaneous correct records',async()=>{
  ...
  await Promise.all(students.slice(2).map(p=>answer(p,correct(0))));await expect(teacher.locator('.ul-group.correct')).toHaveCount(9,{timeout:15000});await expect(teacher.locator('.ul-group.exhausted')).toHaveCount(1);for(const [width,height] of [[1366,768],[1440,1000],[1920,1080]]){await teacher.setViewportSize({width,height});await expect(teacher.locator('.ul-group')).toHaveCount(10);assert.ok(await teacher.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+1),`No horizontal overflow at ${width}x${height}`);const last=teacher.locator('.ul-group').last();await last.scrollIntoViewIfNeeded();await expect(last).toBeVisible();assert.ok(await teacher.locator('.ul-release-login').evaluateAll(buttons=>buttons.every(button=>{const card=button.closest('.ul-group').getBoundingClientRect(),rect=button.getBoundingClientRect();return rect.width>40&&rect.height>=24&&rect.left>=card.left-1&&rect.right<=card.right+1;})),`Release controls fit at ${width}x${height}`);await teacher.screenshot({path:new URL(`teacher-ten-${width}x${height}.png`,out).pathname.replace(/^\/(\w:)/,'$1'),fullPage:true});}await teacher.screenshot({path:new URL('teacher-live-results.png',out).pathname.replace(/^\/(\w:)/,'$1'),fullPage:true});
 });
