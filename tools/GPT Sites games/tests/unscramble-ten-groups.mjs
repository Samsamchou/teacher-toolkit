...
try{
 ...
 await check('Wrong animation, correction, five-attempt cap and nine simultaneous correct records',async()=>{
  ...
  await Promise.all(students.slice(2).map(p=>answer(p,correct(0))));await expect(teacher.locator('.ul-group.correct')).toHaveCount(9,{timeout:15000});await expect(teacher.locator('.ul-group.exhausted')).toHaveCount(1);for(const [width,height] of [[1366,768],[1440,1000],[1920,1080]]){await teacher.setViewportSize({width,height});await expect(teacher.locator('.ul-group')).toHaveCount(10);assert.ok(await teacher.locator('.ul-group').evaluateAll(cards=>cards.every(c=>c.getBoundingClientRect().bottom<=innerHeight)),`All ten groups visible at ${width}x${height}`);await teacher.screenshot({path:new URL(`teacher-ten-${width}x${height}.png`,out).pathname.replace(/^\/(\w:)/,'$1'),fullPage:true});}await teacher.screenshot({path:new URL('teacher-live-results.png',out).pathname.replace(/^\/(\w:)/,'$1'),fullPage:true});
 });
