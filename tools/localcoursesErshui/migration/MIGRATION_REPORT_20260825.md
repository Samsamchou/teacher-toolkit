...
逐檔雜湊與 G 槽讀回證明指定掛載資料夾內的檔案一致。其後再以 Google Drive 連接器直接讀到雲端 `localcoursesErshui` 資料夾、37 個頂層項目，以及 `migration` 子資料夾中的清冊、報告與複製腳本，可確認主要備份已上雲。完整 273 個遠端物件未逐一下載重算雜湊，因此遠端雲端證據屬「資料夾與代表性檔案可見」，完整位元一致性仍以來源／G 槽的 SHA-256 清冊為準。
