...
- G 槽專案共 273 個檔案；`migration\MIGRATION_MANIFEST_20260825.csv` 列 272 個 payload、171,030,048 bytes，逐檔 SHA-256 不一致 0。清冊本身的最終雜湊記在 C 槽轉移指引與 Obsidian，避免清冊所涵蓋的文件反過來自我引用清冊雜湊。
...
- Google Drive 連接器已直接讀到雲端 `localcoursesErshui` 資料夾、37 個頂層項目，並在 `migration` 子資料夾看到清冊、報告與複製腳本；可確認主要備份已上雲。完整 273 個遠端物件未逐一下載重算雜湊，來源／G 槽逐檔 SHA-256 清冊仍是完整一致性證據。
